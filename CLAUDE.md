...
Every `Invoice` carries two distinct identifiers:

- **InternalNumber** (required, unique per company) — our internal system identifier.
  Default format: `YYYYMM{counter:0000}` where `YYYY` is the VAT year, `MM` is the VAT month,
  and `counter` is a zero-padded sequence within that company+month (e.g. `202605001`).
  Generated by `IInvoiceManagementService.GenerateInternalNumberAsync` (which calls
  `IMainInvoiceProvider.GetNextInternalNumberSuffixAsync`, returning `MAX(suffix) + 1`).
  Future per-company custom rules can override this default.
- **OriginalDocumentNumber** (optional, not unique) — the identifier that appears on the
  source document (typically the supplier's invoice number on inbound invoices). Often
  equals `VariableSymbol`, but the two are independent. Used purely for traceability.

Uniqueness is enforced at the DB level via a unique index on `(CompanyId, InternalNumber)`.
On insert collision (race between concurrent imports), the create path catches the unique-
constraint violation, regenerates the suffix, and retries (see
`InvoiceImportService.CreateWithCollisionRetryAsync` and `InvoiceManagementService.CreateWithInternalNumberRetryAsync`).
